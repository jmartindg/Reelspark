...
const getTvShowReviews = async (id) => {
  const res = await axios.get(
    `https://api.themoviedb.org/3/tv/${id}/reviews?api_key=${import.meta.env.VITE_TMDB_API_KEY}&language=en-US`
  );
...
const TvShowReview = () => {
  let { id } = useParams();
  const { data, status } = useQuery(["tvShowReviews", id], () => getTvShowReviews(id), { refetchOnWindowFocus: false });
